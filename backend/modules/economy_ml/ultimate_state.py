...
def build_ultimate_state(previous_round: dict | None, *, puuid: str, agent: str,
    ...
    cost = ultimate.get("ultimate_cost") or ultimate.get("required_points") or ultimate.get("max_charges")
    if points is None:
    ...
    return UltimateState(str(puuid), agent, round_number, True, points, cost,
                         points >= cost if cost else None, "previous_round_player_stats", .8, [])
