...
def main():
    ...
    for name, tag in PLAYERS:
        ...
        print(f"\n[INFO] {riot_id} -> pidiendo {MATCHES_PER_PLAYER} partidas...")

        fetched_match_ids = fetch_match_ids(session, name, tag, MATCHES_PER_PLAYER)
        if not fetched_match_ids:
            print(f"[WARN] Sin partidas para {riot_id} (privado/sin datos/región mal).")
            continue

        match_ids = [mid for mid in fetched_match_ids if mid not in existing_match_ids]
