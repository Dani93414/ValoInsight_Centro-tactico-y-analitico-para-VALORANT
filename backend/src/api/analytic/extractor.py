...
def _finalize_stats_block(stats: dict) -> dict:
    ...
    stats["kd_ratio"] = safe_div(kills, max(deaths, 1))
    stats["kda_ratio"] = safe_div(kills + assists, max(deaths, 1))
    stats["acs"] = safe_div(stats.get("score", 0), rounds)
    stats["adr"] = safe_div(stats.get("damage_dealt", 0), rounds)
    stats["damage_delta_per_round"] = safe_div(stats.get("damage_delta", 0), rounds)
    stats["kills_per_round"] = safe_div(kills, rounds)
    stats["deaths_per_round"] = safe_div(deaths, rounds)
    stats["assists_per_round"] = safe_div(assists, rounds)
    stats["headshot_pct"] = safe_div(stats.get("headshots", 0) * 100.0, total_shots)
    stats["win_rate"] = safe_div(stats.get("wins", 0) * 100.0, rounds)
    stats["survival_rate"] = safe_div(stats.get("survival_rounds", 0) * 100.0, rounds)
    stats["fk_rate"] = safe_div(stats.get("first_kills", 0) * 100.0, rounds)
    stats["fd_rate"] = safe_div(stats.get("first_deaths", 0) * 100.0, rounds)
    stats["fkfd_diff_per_round"] = safe_div(
        stats.get("first_kills", 0) - stats.get("first_deaths", 0),
        rounds,
    )
    opening_total = stats.get("opening_duel_wins", 0) + stats.get("opening_duel_losses", 0)
    stats["opening_duel_win_pct"] = safe_div(stats.get("opening_duel_wins", 0) * 100.0, opening_total)
    stats["trade_kills_per_round"] = safe_div(stats.get("trade_kills", 0), rounds)
    stats["traded_deaths_per_round"] = safe_div(stats.get("traded_deaths", 0), rounds)
    stats["clutch_win_rate"] = safe_div(
        stats.get("clutches_won", 0) * 100.0,
        stats.get("clutch_opportunities", 0),
    )
    stats["multikill_rate"] = safe_div(stats.get("rounds_with_multikill", 0) * 100.0, rounds)
    stats["damage_per_1000_credits"] = safe_div(
        stats.get("damage_dealt", 0) * 1000.0,
        stats.get("econ_spent", 0),
    )
    stats["average_loadout_value"] = safe_div(stats.get("loadout_value_total", 0), rounds)
    ...
    for weapon_id, weapon_stats in stats.get("weapon_stats", {}).items():
        _finalize_weapon_block(weapon_stats)

    return stats


def _finalize_weapon_block(stats: dict) -> dict:
    rounds = stats.get("rounds", 0)
    kills = stats.get("kills", 0)
    deaths = stats.get("deaths", 0)
    assists = stats.get("assists", 0)
