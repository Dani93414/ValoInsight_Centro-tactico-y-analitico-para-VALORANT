...
import { useMatchById, useAgentes } from "../../api/hooks";
import {
  safeDivide,
  formatDateTime,
...
import type { RawPlayer, RawMatchDetail } from "../../types/matches";
...
export default function MatchDetailModal({
  ...
  const loading = matchLoading || agentsLoading;
            ...
            <div className="detail-grid">
              ...
              <section className="detail-card detail-card-large">
                ...
                <div className="rounds-list">
                  {(currentMatch?.roundResults ?? []).map(
                    (round, roundIndex) => {
                      const killEvents = (round.playerStats ?? []).flatMap(
                        (p) =>
                          (p.kills ?? []).map((kill) => ({
                            ...kill,
                            killerPuuid: p.puuid,
                      ...
                      return (
                        <div
                          key={`round-${round.roundNum ?? roundIndex}`}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <small>
                                    ...
                                    ·{" "}
                                    {Math.round(
                                      (kill.timeSinceRoundStartMillis ?? 0) /
                                        1000,
                                    )}
                                    s
                                  </small>
